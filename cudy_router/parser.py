...
def parse_login_screen(input_html: str) -> dict[str, Any]:
    data: dict[str, str] = {}
    soup = BeautifulSoup(input_html, "html.parser")
    inputs = soup.find_all("input")
    for input in inputs:
        if name := input.attrs.get('name', input.attrs.get('id')):
            data[name] = input.attrs.get('value', "")
    return data


def parse_tables(input_html: str, include_headers: bool=False) -> dict[str, Any]:
    ...
    if devices:
        ...
        for device in devices:
            ...
            if mac in device_list:
                data[SECTION_DETAILED][mac] = device
            if hostname in device_list:
                data[SECTION_DETAILED][hostname] = device
            if device_list_str == "*":
                hostname_or_mac = hostname if (hostname and hostname != "Unknown") else mac
                data[SECTION_DETAILED][hostname_or_mac] = device
...
def parse_sms_summary(input_html: str) -> dict[str, Any]:
    """Parses SMS summary"""

    raw_data = parse_tables(input_html, include_headers=True)

    data: dict[str, dict[str, Any]] = {
        'new_messages_count': raw_data.get('New messages'),
        'inbox_count': raw_data.get('Inbox'),
        'outbox_count': raw_data.get('Outbox')
    }
